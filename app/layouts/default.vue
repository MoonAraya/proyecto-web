...
const navegacion = [
    { label: 'Inicio', to: '/' },
    { label: 'Inscripciones', to: '/inscripciones' },
    { label: 'Gestionar Staff', to: '/gestor-cuentas' }, //staff
    { label: 'Gestionar Eventos', to: '/gestor-eventos' } //staff
]
                ...
                <template>
                    <div class="min-h-screen bg-fondo-general">
                        <!-- NAV BAR -->
                        <header>
                            <div class="w-full z-50 bg-fondo-general px-6 py-6 shadow-xl">
                                <!-- para que solo use el contenido dentro de esto -->
                                <div
                                    class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center gap-2 sm:justify-between">
                                    <!-- parte con texto y para poner logo en caso de que haga falta -->
                                    <div>
                                        <h3 class="text-texto font-bold text-lg">Smart Events</h3>
                                    </div>
                    ...
                    <div v-if="user?.rol === 'Administrador'"
                        class="flex flex-col rounded-md border-2 border-boton px-4 py-2">
                        <span class="text-texto text-md">nombre y pellido</span>
                        <span class="text-texto/70 text-sm">rol</span>
                    </div>
                    <!-- 
                    <UButton @click="logout" v-if="user?.rol === 'Administrador'"
                        class="bg-boton text-texto py-2 px-4 rounded-xl hover:bg-boton-hover text-md font-bold transition-colors">
                        ...
                        <footer class="bg-fondo-general py-6">
                            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                                <p class="text-center text-sm text-texto/70">&copy; 2026 Smart Events. Todos los
                                    derechos reservados.
                                </p>
                            </div>
                        </footer>
